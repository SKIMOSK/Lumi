...
class ConversationMemory(private val maxSize: Int = 5) {
    ...
    /** Formats history as Gemini API "contents" array entries (excludes current turn). */
    fun toGeminiContents(): List<Map<String, Any>> {
        ...
        for (interaction in history) {
            ...
            userParts.add(mapOf("text" to interaction.userText))
            interaction.imageBase64?.let { img ->
                userParts.add(mapOf("inlineData" to mapOf(
                    "mimeType" to "image/jpeg",
                    "data" to img
                )))
            }
            contents.add(mapOf("role" to "user", "parts" to userParts))
            contents.add(mapOf("role" to "model", "parts" to listOf(
                mapOf("text" to interaction.assistantText)
            )))
        }
        return contents
    }
